...
class DataBase
{
    ...
    public function getAllRecords( $tableName, $fields = '*', $cond = '', $orderBy = '', $limit = '' )
    {
        ...
        $stmt = $this->pdo->query("SELECT $fields FROM $tableName WHERE 1 " . $cond . " " . $orderBy . " " . $limit);
        //print "SELECT $fields FROM $tableName WHERE 1 ".$cond." ".$orderBy." " ;
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
    }

    public function countRecords( $tableName, $fields = '*', $cond = '' )
    {

        $stmt = $this->pdo->prepare("SELECT $fields FROM  $tableName WHERE $cond");
        //print "SELECT $fields FROM $tableName WHERE 1 ".$cond." ".$orderBy." " ;
        $stmt->execute();
